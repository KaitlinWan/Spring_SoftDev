...
def year(year):
    ...
    for i in (collection.find({"year":year})):
        s += "Motivation: " + i["laureates"][0]["motivation"][1:-1] + "\n"
    return s

# get year and category for surname
def surname(surname):
    s = "Nobel prizes awarded to last name: " + surname + "\n"
...
def num_winners(num):
    s = "Categories and years for which the number of people awarded was: " + num + "\n"
...
def first_name(firstname):
    s = "Nobel prizes awarded to people with the first name: " + firstname + "\n"
